...
class ChatGPTCodex(loader.Module):
    ...
    async def cgauth(self, message: Message):
        ...
        if action == "codex":
            ...
            ok, output = await self._run_codex_device_auth()
    ...
    async def _run_codex_device_auth(self):
        codex_path = self._get_codex_binary()
        if not codex_path:
            return False, "codex not found"
        proc = await asyncio.create_subprocess_exec(
        ...
        return logged_in, status if logged_in else (output or status or f"exit={proc.returncode}")
